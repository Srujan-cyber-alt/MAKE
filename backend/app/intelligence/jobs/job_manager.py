...
class JobRunner:
    ...
    def __init__(
        ...
        self._log = log_fn or JobRunner._noop_log
        self._cancelled = False

    @staticmethod
    async def _noop_log(level: str, msg: str, **kw) -> None:
        return None
    ...
    async def run_step(
        ...
        await self._log("INFO", f"Executing step {step.id}: {step.description}", step_id=step.id)
...
class JobManager:
    ...
    async def execute_job(self, job: IntelligenceJob) -> Dict[str, Any]:
        ...
        runner = JobRunner(
            ...
            log_fn=lambda level, msg, **kw: self._log(job.id, level, msg, extra=kw),
        )
