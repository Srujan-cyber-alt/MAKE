...
def cmd_train(args: argparse.Namespace) -> int:
    ...
    print(f"Layers: {cfg.num_layers}, Hidden: {cfg.hidden_dim}, Heads: {cfg.num_heads}")

    hw = detect_hardware()
    print(f"Hardware: {hw.gpu_name or 'CPU-only'}")

    # Enforce hardware gate for production
    if arch_preset == "PRODUCTION" and not hw.has_cuda and not args.force_cpu:
        print("[BLOCKED] Production 5B training requires CUDA GPU")
        print("Use --force-cpu only for smoke testing")
        return 1
